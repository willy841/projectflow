...
## 3. 工種（vendor classification）管理

### 使用情境
Vendor Data 需要支援對 `工種` 進行：
1. 新增
2. 編輯
3. 刪除
...
### 判斷
- 功能邏輯：正確
- 工種應視為可管理的 vendor classification master，而不是自由文字欄位
- 與 Vendor Data 作為 vendor master 管理入口的責任一致

### 補充風險說明
- `新增 / 編輯`：合理且必要
- `刪除`：屬敏感操作，後續需更嚴格定義是否允許刪除已被既有 vendor 使用中的工種

---

## 4. Vendor Detail：廠商資訊（可編輯）
...
2. 工種應作為可管理的 vendor classification master，且目前為單選
3. vendor detail 可編輯 vendor profile
4. vendor detail 需有勞報資訊 Tab
5. 未付款專案承接同 vendor 在單專案下的設計 / 備品 / 廠商未付款加總（不含人工）
6. `已付款` 是 unpaid -> history 的狀態切換
7. 往來紀錄應分為 `未結帳` / `過往紀錄`
8. 往來紀錄每筆資料可 inline 展開查看具體合作項目與需求內容
9. Inline 展開內容屬於文件層 readback，且不可編輯
10. 往來紀錄需具備時間排序與搜尋框
...
9. 工種刪除是否需要被限制於沒有既有 vendor 綁定的工種類別
