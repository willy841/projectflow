...
> **本章閱讀規則（2026-04-08 補充）：**
> 本章以下內容混有「現行仍有效的結構規則」與「保留供回查的歷史脈絡」。
> 後續若要續接 `projectflow`，請優先依下列順序判讀：
> 1. 本章中已明確標示為 `最新`、`正式定位`、`正式規則`、`已拍板` 的段落
> 2. `MD21`～`MD26`、`MD34`、`MD35`
> 3. `MD38`～`MD51`（下游 / Accounting Center 主線）
>
> 正式原則：
> - 本章中凡明確標示為 `歷史脈絡保留`、`較早期定稿`、`若衝突以新版 spec 為準` 的段落，不可再當成現行主線直接執行。
> - 若內容與 `MD40`、`MD44`、`MD49`、`MD50` 等最新母 spec 衝突，一律以較新的母 spec 為準。
...
> **本章狀態註記（2026-04-08 補充）：**
> 本章仍保留大量 `2026-04-03`～`2026-04-04` 的高價值產品脈絡，但它已不是目前下游主線的唯一最新入口。
>
> 目前正確判讀方式：
> - 與 `報價成本 / 結案 / Vendor / 管銷成本 / 營收概況` 有關的續接，不可只讀本章。
> - 若涉及下游最新主線，必須同步回看：
>   - `MD35`
>   - `MD38`～`MD40`
>   - `MD41`
...
>
> 正式原則：
> - 本章可作為 `報價成本`、`結案`、`vendor 財務承接` 的歷史形成脈絡與規則來源。
> - 但若本章內容與 `MD40`（Closeout）、`MD44`（管銷成本）、`MD49`（營收概況）、`MD50`（Accounting Center 母整合）衝突，以較新的母 spec 為準。
...
> **本章狀態註記（2026-04-08 補充）：**
> 本章適合作為快速理解 `projectflow` 模組版圖的高階導覽，但其中對 `Accounting Center` 與下游模組的描述，已被本輪最新整合文件進一步正式化。
>
> 後續若要續接下游主線，請不要只停在本章，必須補讀：
...
> 正式原則：
> - 本章保留為模組地圖與歷史總覽。
> - 具體的 `Accounting Center` 結構、子區規格、營收概況與跨模組對齊，應以 `MD50` / `MD51` 與各子 spec 為準。
...
## 20.15 任務發佈區 Excel 匯入升級（2026-04-04 下午更新）
本節補記 `任務發佈區` 從 CSV 匯入升級為『報價單式 Excel 解析匯入』的第一版落地結果。若後續續接 execution tree、任務發佈、報價單轉任務草稿，應以本節為準。

### A. 產品定位
本輪已正式改變匯入心智：
- 不再是單純 `CSV 模板匯入`
- 改為 **從報價單式、半結構化 Excel 中解析任務結構**
- 正式定位應理解為：
  > **報價單轉任務草稿匯入器 v1**
...
3. 只吃第一個 sheet
4. 匯入前先預覽 / 確認
5. 系統自動找第一塊符合報價明細表頭的有效表格區
6. 第一個大方案名稱也可視為主項目
7. 主項目列獨立一列，子項目列在下面
8. 子項目延續列併到上一筆子項目摘要
9. 單價 / 數量 / 單位 / 金額只顯示在預覽供核對，不作正式任務欄位
10. 一旦進入小計 / 稅金 / 總金額 / 備註 / 付款條件 / 匯款資訊等結算或條款區，後續停止解析
...
正式語意：
> 這不是完全自由格式匯入，而是對「報價單式 Excel」做一組可控規則解析。

### D. 預覽 / 確認流程
本輪已改成：
- 上傳 Excel 後先產生預覽
- 不直接覆蓋任務樹
- 只有按下 `確認匯入` 後，才正式寫入任務發佈區
...
- `src/components/excel-task-import.ts`
...
- 不做多種報價單格式全兼容
- 不把這輪變成正式財務匯入器
- 目前子項目來源中的金額資訊仍可能以 note 形式短暫保留於內部資料，若之後要更嚴格，可再收掉
