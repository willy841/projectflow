...
- `tests/formal-acceptance-v2/25-project-core-owner-cross-page-consistency.spec.ts`
- `tests/formal-acceptance-v2/26-design-assignee-cross-page-consistency.spec.ts`
- `tests/formal-acceptance-v2/27-project-list-budget-cost-source-of-truth.spec.ts`
- `tests/formal-acceptance-v2/28-reconciliation-group-integrity.spec.ts`
- `tests/formal-acceptance-v2/29-vendor-unpaid-lifecycle-cross-page.spec.ts`
- `tests/formal-acceptance-v2/30-dispatch-family-routing-downstream-readback.spec.ts`
- `tests/formal-acceptance-v2/31-collections-downstream-summary-pack.spec.ts`
- `tests/formal-acceptance-v2/32-closeout-active-archive-consistency-pack.spec.ts`

### 什麼時候用這條
若問題是：
- 這輪 UI / code 有沒有把正式功能改壞？
- vendor / quote-cost / closeout / cross-flow 主線還在不在？
- source-of-truth / cross-page consistency / downstream lifecycle 有沒有被破壞？
- 要不要判定 blocker？
...
- `tests/formal-acceptance-v2/25~32` 已正式納入 `test:formal-acceptance:v2` 與 `test:formal-acceptance:full`。
- 這代表 acceptance 主入口現在不只驗 mainline clickpath，也正式開始驗：
  - project/detail/list/home 欄位級 readback
  - financial source-of-truth list sync
  - reconciliation status+amount completeness
  - vendor unpaid increase/decrease lifecycle
  - dispatch family routing downstream readback
  - collections downstream summary truth
  - closeout active/archive consistency

## 7. 一句話版本

> 正式 blocker 一律先看 `tests/formal-acceptance-v2/`；而且自 2026-05-05 起，這條入口已正式包含 A~H cross-page consistency packs。`tests/legacy/stale-vendor-ui-contract/` 只代表舊契約殘留，不代表現在正式主線壞掉。
