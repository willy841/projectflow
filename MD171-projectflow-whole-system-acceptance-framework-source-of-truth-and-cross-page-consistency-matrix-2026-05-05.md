...
- `MD171` = 全站驗收體系母檔（原則 / 分層 / 模組級矩陣）
- `MD172` = 缺口矩陣（哪些已補、哪些仍缺）
- `MD173` = regression pack 規格（應補哪些 pack、如何補）

2026-05-05 最新狀態：
- `MD173` 第一輪定義的 Pack A ~ H，已正式落地成 `tests/formal-acceptance-v2/25~32`
- 並已正式納入：
...
這一層原本是 `projectflow` 最缺的制度化層。  
2026-05-05 起，第一輪 A~H packs 已正式落地進入口；後續應在這個基礎上繼續往第二輪與第三輪擴充，而不是回退成只看 mainline clickpath。  
對應文件：`MD173`。
...
### A. `MD172-projectflow-source-of-truth-test-gap-matrix-2026-05-05.md`
- 列每條資料欄位 / 聚合值 / status transition
- 對照現有 test coverage
- 標記缺口與優先級

### B. `MD173-projectflow-cross-page-consistency-regression-pack-spec-2026-05-05.md`
- 列出應補的具體 E2E / API / DB readback tests
- 第一輪 Pack A ~ H 已落地成 `25~32`

下一步不再是「要不要開始補 A~H」，而是：
1. 維護 `MD172` 讓缺口判讀跟實際 coverage 同步
2. 規劃第二輪 deeper coverage（home summary / accounting deeper UI / adapter drift / manual spot-check checklist）
