          ...
          projectTasks.length ? (
            <div className="pf-table-shell rounded-[28px]">
              <table className="pf-table min-w-[980px] table-fixed xl:min-w-full">
                <thead>
                  <tr>
                    <th className="w-[42%] px-4 py-3 font-medium align-middle">任務標題</th>
                ...
                <tbody className="divide-y divide-white/10 bg-transparent">
                  {projectTasks.map((task) => (
                    <tr key={task.id} className="align-middle">
                      <td className="px-4 py-4 font-medium text-slate-100"><span className="block truncate" title={task.title}>{task.title}</span></td>
                      ...
                      <td className="px-4 py-4 text-center">
                        <div className="flex justify-center whitespace-nowrap">
                          <Link href={`/procurement-tasks/${task.id}`} className={`${workspacePrimaryButtonClass} whitespace-nowrap`}>進入任務</Link>
                        </div>
                      </td>
                    </tr>
                  ))}
