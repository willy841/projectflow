    ...
    <AppShell activePath="/projects">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="mb-5 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">內部逐項管理單位，掛在 execution item 上，不直接代表正式發包。</p>
            </div>
            <button className="text-sm font-medium text-slate-700">+ 新增 Vendor Assignment</button>
          </div>
        ...
        <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <h3 className="text-xl font-semibold">Vendor Packages</h3>
              <p className="mt-1 text-sm text-slate-500">同專案 + 同廠商的對外發包彙整單，也是正式發包主體。</p>
            </div>
            <button className="text-sm font-medium text-slate-700">查看全部包單</button>
          </div>

          <div className="space-y-3">
            {vendorPackages.length ? (
              vendorPackages.map((vendorPackage) => (
                <div key={vendorPackage.id} className="rounded-2xl border border-slate-200 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">包內項目：{vendorPackage.assignmentIds.length} 筆</p>
                    </div>
