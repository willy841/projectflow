...
type QuoteCostListMode = "active" | "closed";

const pageConfig = {
  active: {
    activePath: "/quote-costs",
    pageEyebrow: "Quote & Cost Module",
    pageTitle: "報價成本",
    pageDescription: "以專案為主體管理對外報價、成本、毛利與對帳。此列表頁只保留執行中專案。",
    listTitle: "執行中專案",
    listDescription: "進行中的專案集中在此管理報價、成本與對帳。",
    detailBasePath: "/quote-costs",
  },
  closed: {
    activePath: "/closeouts",
    pageEyebrow: "Project Closeout Module",
    pageTitle: "結案",
    pageDescription: "已結案專案獨立收斂於此，延續既有報價成本詳情結構查看最終報價、成本與對帳結果。",
    listTitle: "已結案專案",
    listDescription: "此列表只顯示已結案專案，供結案後追蹤與查閱。",
    detailBasePath: "/closeouts",
  },
} as const;

export function QuoteCostListClient({ mode = "active" }: { mode?: QuoteCostListMode }) {
  ...
  const visibleProjects = mode === "closed" ? closedProjects : activeProjects;
  ...
  return (
    <AppShell activePath={config.activePath}>
      <header className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 xl:p-7">
        ...
        <h2 className="mt-1 text-3xl font-semibold tracking-tight text-slate-900">{config.pageTitle}</h2>
        <p className="mt-3 max-w-4xl text-sm leading-6 text-slate-600">{config.pageDescription}</p>
      </header>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div className="mb-5 flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h3 className="text-xl font-semibold text-slate-900">{config.listTitle}</h3>
              ...
              {visibleProjects.map((project) => {
                ...
                return (
                  <tr key={project.id} className="align-middle">
                    ...
                    <td className="px-4 py-4">
                      <Link
                        href={`${config.detailBasePath}/${project.id}`}
