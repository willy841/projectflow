  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex flex-wrap items-center gap-3"><button type="button" onClick={addPlan} className="inline-flex min-h-11 items-center justify-center rounded-2xl border border-white/10 bg-slate-900/50 px-4 py-2.5 text-sm font-semibold text-slate-100 shadow-[0_22px_46px_-28px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl transition hover:bg-slate-900/70">新增執行處理</button></div>
        <div className="flex flex-wrap items-center gap-2"><button type="button" onClick={saveAllPlans} disabled={saving} className="inline-flex min-h-11 items-center justify-center rounded-2xl border border-white/10 bg-slate-900/50 px-4 py-2.5 text-sm font-medium text-slate-100 shadow-[0_22px_46px_-28px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl transition hover:bg-slate-900/70 disabled:opacity-60">{saving ? "儲存中..." : "儲存"}</button>{showConfirmButton ? <button type="button" onClick={confirmPlans} disabled={confirming} className="inline-flex min-h-11 items-center justify-center rounded-2xl border border-emerald-300/20 bg-[linear-gradient(135deg,rgba(5,150,105,0.88),rgba(6,95,70,0.92))] px-4 py-2.5 text-sm font-semibold text-white shadow-[0_24px_48px_-24px_rgba(5,150,105,0.5),inset_0_1px_0_rgba(255,255,255,0.12)] backdrop-blur-xl transition hover:brightness-110 disabled:opacity-60">{confirming ? "確認中..." : "全部確認"}</button> : null}</div>
      </div>
      ...
      {plans.map((plan, index) => (
        <article key={plan.id} className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(31,41,59,0.72),rgba(15,23,42,0.58))] p-5 shadow-[0_24px_56px_-32px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-2xl">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <label className="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3.5"><p className="text-xs font-medium tracking-wide text-slate-400">標題</p><input value={plan.title} onChange={(e) => updatePlan(plan.id, "title", e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
            <label className="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3.5 md:col-span-2"><p className="text-xs font-medium tracking-wide text-slate-400">需求說明</p><textarea value={plan.requirement} onChange={(e) => updatePlan(plan.id, "requirement", e.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 py-2 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
