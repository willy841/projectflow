      ...
      {visiblePlans.map((plan) => (
        <article key={plan.id} className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            ...
            <label className="xl:col-span-2"><p className="text-xs font-medium tracking-wide text-slate-400">預覽連結</p><input value={plan.previewUrl} onChange={(e) => updatePlan(plan.id, "previewUrl", e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
            <label className="xl:col-span-2"><p className="text-xs font-medium tracking-wide text-slate-400">執行廠商</p><input list={vendorListId} value={plan.vendor} onChange={(e) => updatePlan(plan.id, "vendor", e.target.value)} placeholder={vendorOptions.length ? "可直接輸入或選擇既有廠商" : "可直接輸入廠商名稱"} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
          </div>
