...
export function ProcurementPlanEditorClient({
  ...
  useEffect(() => {
    externalHeaderActions?.({
      ...
      removeSelectedPlan: () => {
        if (selectedPlanId) void removePlan(selectedPlanId);
      },
      saving,
    });
  ...
  function addPlan() {
    ...
    setPlans((current) => [
      { id: nextId, title: "", quantity: "", amount: "", previewUrl: "", vendor: "" },
      ...current,
    ]);
    onSelectPlanId?.(nextId);
  }

  async function replacePlans(nextPlans: ProcurementPlanInput[]) {
    const currentPlans = nextPlans.filter((plan) => [plan.title, plan.quantity, plan.amount, plan.previewUrl, plan.vendor].some((value) => value.trim()));
  ...
  async function removePlan(id: string) {
    ...
    try {
      setSaving(true);
      const persistedRows = await replacePlans(next);
      if (selectedPlanId === id) {
  ...
  async function persistCurrentPlans() {
    ...
    const persistedRows = await replacePlans(plans);
  ...
  async function confirmPlans() {
    ...
    try {
      ...
      await persistCurrentPlans();
      const response = await fetch(`/api/procurement-tasks/${taskId}/confirm`, { method: "POST" });
      if (!response.ok) {
        const payload = (await response.json().catch(() => null)) as { error?: string } | null;
  ...
  return (
    <div className="space-y-4">
      {!hideTopActions ? (
        <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={confirmPlans} disabled={confirming} className="inline-flex min-h-11 items-center justify-center rounded-2xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">{confirming ? "確認中..." : "全部確認"}</button>
          </div>
        </div>
      ) : null}
      ...
      {visiblePlans.map((plan, index) => (
        <article key={plan.id} className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(39,52,77,0.62),rgba(17,26,42,0.46))] p-5 shadow-[0_24px_46px_-28px_rgba(0,0,0,0.54),0_0_16px_rgba(96,165,250,0.05),inset_0_1px_0_rgba(255,255,255,0.05),inset_0_-16px_24px_-18px_rgba(10,18,32,0.86)] backdrop-blur-2xl">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <label className="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3.5 xl:col-span-2"><p className="text-xs font-medium tracking-wide text-slate-400">標題</p><input value={plan.title} onChange={(e) => updatePlan(plan.id, "title", e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
            <label className="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3.5"><p className="text-xs font-medium tracking-wide text-slate-400">數量</p><input value={plan.quantity} onChange={(e) => updatePlan(plan.id, "quantity", e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
            <label className="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3.5"><p className="text-xs font-medium tracking-wide text-slate-400">金額</p><input value={plan.amount} onChange={(e) => updatePlan(plan.id, "amount", e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
            <label className="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3.5 xl:col-span-2"><p className="text-xs font-medium tracking-wide text-slate-400">預覽連結</p><input value={plan.previewUrl} onChange={(e) => updatePlan(plan.id, "previewUrl", e.target.value)} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
            <label className="rounded-2xl border border-white/10 bg-slate-950/35 px-4 py-3.5 xl:col-span-2"><p className="text-xs font-medium tracking-wide text-slate-400">執行廠商</p><input list={vendorListId} value={plan.vendor} onChange={(e) => updatePlan(plan.id, "vendor", e.target.value)} placeholder={vendorOptions.length ? "可直接輸入或選擇既有廠商" : "可直接輸入廠商名稱"} className="mt-2 h-10 w-full rounded-xl border border-white/10 bg-slate-950/50 px-3 text-sm font-medium text-slate-100 outline-none focus:border-sky-400/40 focus:ring-2 focus:ring-sky-400/20" /></label>
          </div>
      ...
      {message ? <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-sm whitespace-pre-line text-emerald-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl">{message}</div> : null}
    </div>
  );
}
