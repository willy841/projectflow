...
type Props = {
  ...
  initialRecords: VendorProjectRecord[];
};

export function VendorDetailShell({ vendor, initialRecords }: Props) {
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">{vendor.note}</p>
          </div>
      ...
      <section className="grid gap-6 2xl:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
        <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {[
              ["合作類型", vendor.category],
              ["聯絡人", vendor.contactName],
              ["電話", vendor.phone],
              ["Email", vendor.email],
              ["LINE", vendor.lineId],
              ["地址", vendor.address],
              ["銀行", `${vendor.bankName} (${vendor.bankCode})`],
        ...
        <article className="rounded-3xl border border-amber-200 bg-amber-50/60 p-6 shadow-sm ring-1 ring-amber-100">
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-xs font-semibold tracking-wide text-amber-700">B. 未付款專案區</p>
      ...
      <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div className="mb-5">
          <p className="text-xs font-semibold tracking-wide text-slate-500">C. 往來 / 歷史紀錄區</p>
