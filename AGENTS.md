...
## Projectflow 啟動入口（2026-05-12 封裝版）

只要任務涉及 `projectflow` / `project-mgmt`，先讀：
...
- 以及其指向的治理、deployment、runtime、DB、handoff 文件

若任務不是 `projectflow`，不可主動載入這整包內容，也不可讓它污染其他一般對談。

## Projectflow 續接必讀順序（2026-05-11 治理架構版）

凡是要續接 `projectflow`，不可只憑印象、不可只翻單一 handoff、也不可直接從早期 spec 或 UI 檔起手。

### 強制規則
- **每次開啟新對話 / 新 session，只要要碰 `projectflow`，都必須先讀：**
...
- 不可跳過這五份入口，直接讀單一 handoff、單一 spec 或單一 UI 檔。
- 若新舊文件衝突，優先依上述五份入口與其指向的 Active 主線文件判斷。

### 新 session 的最小安全讀法
在五份治理入口之後，至少再讀：

6. `MD155-projectflow-single-track-acceptance-first-and-production-freeze-rule-2026-04-20.md`
7. `MD156-projectflow-project-document-routing-and-dispatch-trade-linkage-rules-2026-04-24.md`
8. `MD157-projectflow-vendor-financial-source-formalization-and-vendor-detail-performance-investigation-2026-04-24.md`
...
如果要碰驗收 / source-of-truth / cross-page consistency：
...
### 對新 agent / 新 session 的正式判讀
- `projectflow` **現在是正式成熟系統，不是早期探索期。**
- 現在主線不是「還要不要先把頁面做起來」，而是：
  - 現行規則維持
  - 成熟度判讀
  - 全站驗收體系
  - formal truth 收斂
  - local / fallback / bridge 殘留鏈退休
- `MD203~210` 不是旁支雜訊，而是現在工程主線的一部分。

### 明確禁止事項
- 不可只讀 `MD21`、`MD26` 或任何早期 handoff 就開始判斷現在規則。
- 不可依賴舊文件中的閱讀鏈，因為部分引用檔在目前 workspace 中已缺失（見 `MD-INDEX-projectflow-missing-legacy-reference-audit-2026-05-11.md`）。
- 不可把 Historical 層文件當主入口。
...
重要：
- 這些檔案仍可保留作歷史脈絡、產品語意來源、或早期 repo 現況回查
- 但不可再用來主導新 session 的現在式判讀
...
### Vendor Detail / Shell UI 色差問題（2026-05-11 經驗規則）

這條是已踩過坑的明確規則，後續不可再從錯層級重查太久。

- 若在 `projectflow` 的 vendor detail 或相似頁面看到：
  - 底部左右角色差
  - 側邊或主內容外緣出現亮帶 / 藍帶 / 不同材質帶
  - 看起來像內容卡片問題，但位置其實貼近 layout 邊界
- **先查 layout shell，不要先鑽內文元件。**
- 第一優先檢查檔案：`project-mgmt/src/components/auth-shell-client.tsx`
- 這類問題常見根因：
  - `main` 背景
  - 左側 `aside` 與右側 `section` 背景不一致
  - 底部角落露出 body / page background glow
  - shell stretch / min-height / bottom surface 沒對齊
- **不要優先從** `project-mgmt/src/components/vendor-detail-shell-db.tsx`、table、header、article、內文 card 開始亂追，除非已先排除 shell。
- 這次真正修正成功的提交是：`44039dd` (`fix(projectflow): unify shell and page background at bottom edges`)
- 若未來再遇到相似症狀，應先對照這次修法，再決定是否需要更深層 debug。

### Shell Source Misidentification（2026-05-11 追加規則）

這是另一個已踩過的坑：**不只是查錯層，還可能直接改錯 shell 檔案。**

- 若某頁面的底角 / 背景 / 外殼問題「怎麼改都完全沒效果」，不要只懷疑顏色值或 class；要懷疑：
  - 目前改的 shell 檔案根本不是實際生效的那份
- **強制規則：先查 rendered DOM，再判斷實際命中的 shell source。**
- 不可只因為頁面看起來像 auth shell，就直接改 `auth-shell-client.tsx`。
- 至少要先確認實際命中的是：
  - `project-mgmt/src/components/auth-shell-client.tsx`
  - 還是 `project-mgmt/src/components/app-shell.tsx`
  - 或其他 layout / shell
- 若 rendered DOM class 與 source 不符，優先以 rendered DOM 追 shell source，不可繼續盲改。
- `quote-cost` dark-glass 底角問題這次真正生效的修正位置是：
  - `project-mgmt/src/components/app-shell.tsx`
  - 成功提交：`f442646` (`fix(projectflow): harden dark-glass app shell surfaces`)
- 之後只要連續多次 style patch 都「完全沒效果」，應立刻啟動這條規則，不可再盲改同一份 shell。
